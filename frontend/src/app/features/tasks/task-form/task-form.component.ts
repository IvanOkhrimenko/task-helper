            ...
            <div class="form-group">
              ...
              @if (googleAccounts().length > 0) {
                <select
                  ...
                  class="form-input"
                >
                  <option [ngValue]="null">None - Don't create drafts automatically</option>
                  @for (account of googleAccounts(); track account.id) {
                    <option [ngValue]="account.id">{{ account.email }}</option>
                  }
                </select>
...
export class TaskFormComponent implements OnInit {
  ...
  form = this.fb.nonNullable.group({
    ...
    googleAccountId: [null as string | null]
  });
  ...
  ngOnInit() {
    ...
    // Fetch Google accounts for the selector
